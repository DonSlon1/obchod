...
if (!empty($rating) && 5 >= $rating && $rating >= 1) {
    if (array_key_exists("img_type", $_POST)) {
        if ($_POST["img_type"] == "image/jpeg" || $_POST["img_type"] == "image/png") {
            $ID_O = generateRandomString(10) . "." . (mb_substr($_POST["img_type"], mb_stripos($_POST["img_type"], "/") + 1));
            ...
            $stmt->bind_param("iisisss", $id_u, $id_p, $popis, $rating, $positive, $negative, $ID_O);
            $stmt->execute();
            $_SESSION["safa"] = true;
            $_SESSION["ID_O"] = $ID_O;
        } else {
        ...
        $stmt = $con->prepare($sql);
        $stmt->bind_param("iississs", $id_u, $id_p, $popis, $rating, $positive, $negative, $ID_O);
        $stmt->execute();
        unset($_SESSION['csrf_token']);
